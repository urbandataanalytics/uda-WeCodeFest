...
public class WordsCountWithTop {
    ...
    public static void main(String[] args) {
        ...
        p.apply(TextIO.read().from("data/corpus.txt"))
         ...
         .apply("get Top Words", Top.of(10, new TopWords()))
         .apply("Top flatten", Flatten.iterables())
         .apply("get Strings", MapElements.into(TypeDescriptors.strings()).via((KV<String, Long> pair) -> pair.getKey().toString()+" -> "+pair.getValue().toString()))
         .apply("Write", TextIO.write().to("WordCountResult").withoutSharding());
